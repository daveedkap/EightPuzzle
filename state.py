...
class State:
    ...
    def is_goal(self):
        """ returns True if the called state is equal to goal state and False if otherwise
        """
        return self.board.tiles == GOAL_TILES
    
    def generate_successors(self):
        """ creates and returns a list of State object for all successor states of the called State object
        """
        succesors_list = []
        for m in MOVES:
            new_board = self.board.copy()
            if new_board.move_blank(m) == True:
                succesors_list += [State(new_board, self, m)]
    ...
    def print_moves_to(self):
        """ prints the sequences of moves that lead from the initial state to the called State object
        """
        if self.predecessor == None:
            print('initial state:')
            print(self.board)
